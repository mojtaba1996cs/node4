...
app.use(function(request, response, next) {
    ...
    if (session_id) {
        connection.query("SELECT users.id, users.username, full_name, is_author FROM `users` JOIN `sessions` ON users.username=sessions.username WHERE session_id=?", [ session_id ], function(err, row){
            if (!err && row[0]){
                request.user = row[0];
            }
...
app.get("/", function(request, response) {
    connection.query("SELECT * from `posts` ORDER BY date DESC LIMIT 10;", function(err, posts) {
        response.render("home", { posts: posts, formatDate: formatDate , user:request.user }); });   
});
